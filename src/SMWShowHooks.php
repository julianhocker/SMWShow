...
class SMWShowHooks {
   ...
   public static function onParserFirstCallInit( Parser $parser ) {
      ...
      $parser->setFunctionHook( 'smwshow', [ self::class, 'smwshow' ] );
   ...
   public static function smwshow( Parser $parser, $param1 = '', $param2 = '', $param3 = '') {
        global $wgScriptPath;
        global $wgServer;
		...
		//if (empty($param1)){#check, if input is empty. If it is not, get wikidata-id from api
			...
			//https://schularchive.bbf.dipf.de/api.php?action=ask&query=[[Gymnasium_Leopoldinum_Passau]]|?Schulort|limit=5&format=json
			$title = str_replace(" ","_",$param2);
			$attribute = str_replace(" ","_",$param3);
			$url = "$param1?action=ask&query=[[$title]]|?$attribute|limit=5&format=json";
            $json_data = file_get_contents($url);
            $apiresponse = json_decode($json_data, true);
			#handling pages where wikidaalink is not defined:

			try {
                  if (empty($apiresponse['query']['results'][$param2]['printouts'][$param3][0])){
                    throw new Exception("not defined");
                }else {
			        if(empty($apiresponse['query']['results'][$param2]['printouts'][$param3][0]['fullurl'])){
			            $wikidataentry = $apiresponse['query']['results'][$param2]['printouts'][$param3][0];#get wikidatalink from api
			        } else {
			            $wikidataentry = $apiresponse['query']['results'][$param2]['printouts'][$param3][0]['fulltext']; ##TODO: Bug, not working
			        }
                }
            }
             //catch exception
            catch(Exception $e) {
                return "No wikidata entry found $url";
            }
		//}else{
